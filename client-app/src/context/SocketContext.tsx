...
import { generateAndStoreKeys, loadStoredKeys, saveMessageLocally, loadLocalHistory, loadLocalInbox, normalizeUsername } from '../crypto/keyManagerFileSystem';
...
export const SocketProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const syncOwnKeysWithServer = useCallback(async (username: string): Promise<void> => {
        ...
        if (!localKeys) {
            console.log('[Key Sync] No local keys found. Generating new keys...');
            ...
            console.log('[Key Sync] New keys generated and uploaded to server.');
            return;
        }
        ...
        if (!serverKeys) {
            console.log('[Key Sync] No keys found on server. Uploading local keys...');
            socket.emit('upload_public_keys', {
                username: normalizedUsername,
            ...
            console.log('[Key Sync] Local keys uploaded to server.');
            return;
        }
        ...
        if (!signingMatches || !encryptionMatches) {
            ...
            console.log('[Key Sync] Local keys uploaded to server (mismatch resolved).');
        } else {
            console.log('[Key Sync] Keys are in sync with server.');
        }
    ...
    useEffect(() => {
        ...
        if (savedToken && savedUsername) {
            ...
            // Check if we have keys locally. If not, generate them and upload to server.
            loadStoredKeys(savedUsername).then(async (existingKeys) => {
                try {
                    if (!existingKeys) {
                        console.log('[Key Recovery] No local keys found. Generating new keys...');
                        const newKeys = await generateAndStoreKeys(savedUsername);
                        
                        // Upload new keys to server
                        newSocket.emit('upload_public_keys', {
                            username: savedUsername,
                            signingPublicKey: newKeys.signingPublicKey,
                            encryptionPublicKey: newKeys.encryptionPublicKey,
                            format: newKeys.format,
                        });
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            const onSuccess = async (data: { token: string; username: string }) => {
                try {
                    ...
                    const existingKeys = await loadStoredKeys(data.username);
                    
                    if (!existingKeys) {
                        console.log('[Login] No local keys found. Generating new keys...');
                        const newKeys = await generateAndStoreKeys(data.username);
                        ...
                        console.log('[Login] New keys generated and uploaded to server.');
                    } else {
                        console.log('[Login] Existing keys found. Syncing with server...');
