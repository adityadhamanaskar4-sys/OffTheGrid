...
import { createCipheriv, createDecipheriv, createPrivateKey, createPublicKey, diffieHellman, generateKeyPairSync, randomBytes, createHash } from 'node:crypto';
...
const normalizeBase64 = (value) => (value || '').replace(/\s+/g, '').trim();
...
ipcMain.handle('crypto-encrypt-message', async (_event, payload) => {
    try {
...
ipcMain.handle('crypto-decrypt-message', async (_event, payload) => {
    try {
